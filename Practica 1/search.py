...
class Problem:
    ...
    """
    OBLIGAMOS A QUE TODOS LOS PROBLEMAS TENGAN UNA HEURISTICA. SI NO SE SOBREESCRIBE EL METODO
    DEVOLVEMOS 0 (ES COMO SI NO HUBIESE HEURISTICA)
    """
    def h(self, node):
        """Se encarga de darnos la Heuristica para un determinado nodo en un tipo de
        problema concreto"""
        return 0
...
"""
METODO MODIFICADO PARA QUE CUENTE NODOS EXPANDIDOS EN CADA METODO
"""
...
"""
BUSQUEDA POR RAMIFICACION Y ACOTACION (TREE_SEARCH = NO USAMOS LISTA CERRADA EN ESTE TIPO DE BUSQUEDA)
"""
def ramificacion_y_acotacion(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""
    return tree_search(problem, MyFirstList())

"""
BUSQUEDA POR RAMIFICACION Y ACOTACION CON SUBESTIMACION (TREE_SEARCH = NO USAMOS LISTA CERRADA EN ESTE TIPO DE BUSQUEDA)
"""
def ramificacion_y_acotacion_sub(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""
    return tree_search(problem, MyFirstList_sub(problem))
