...
"""
LISTA PERSONALIZADA PARA QUE ORDENE POR PATHCOST SIEMPRE QUE SE EXTIENDA UN NODO
"""
class MyFirstList(Queue):
    """A First-In-First-Out Queue."""
...
"""
- LISTA PERSONALIZADA PARA QUE ORDENE POR (PATHCOST + HEURISTICA) SIEMPRE QUE SE EXTIENDA UN NODO
- HEREDA DE LA CLASE MyFirstList SOBREESCRIBIENDO EL METODO DE ORDENACION (DIFERENTE ORDENACION)
"""
class MyFirstList_sub(MyFirstList):
    """A First-In-First-Out Queue."""

    def __init__(self, problem):
        self.A = []
        self.start = 0
        # REFERENCIA A OBJETO DE TIPO DE PROBLEMA PARA ACCEDER AL METODO QUE CALCULA LA HEURISTICA ESPECIFICA DEL MISMO
        self.p = problem

    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key = lambda x: (x.path_cost + (self.p).h(x)))
